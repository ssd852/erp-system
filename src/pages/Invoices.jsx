...
const Invoices = () => {
    ...
    const [formData, setFormData] = useState({ customer_id: '', date: '', due_date: '', total_amount: 0, tax_rate: 0, amount_paid: 0, status: 'Pending' });
    ...
    const openAdd = () => {
        setFormData({ customer_id: '', date: new Date().toISOString().split('T')[0], due_date: '', total_amount: 0, tax_rate: 0, amount_paid: 0, status: 'Pending' });
        setIsEditing(false);
        setShowModal(true);
    };

    const openEdit = (item) => {
        setFormData({ id: item.id, customer_id: item.customer_id || '', date: item.invoice_date || item.date || '', due_date: item.due_date || '', total_amount: item.total_amount || 0, tax_rate: item.tax_rate || 0, amount_paid: item.amount_paid || 0, status: item.status || 'Pending' });
        setIsEditing(true);
        setShowModal(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            tax_rate: parseFloat(String(formData.tax_rate || 0).replace(/[^0-9.-]+/g, '')) || 0,
            amount_paid: parseFloat(String(formData.amount_paid || 0).replace(/[^0-9.-]+/g, '')) || 0,
    ...
    const statusBadge = (s) => {
        if (s === 'Paid') return 'bg-emerald-500/15 text-emerald-400';
        ...
        <div className="p-6 w-full" dir="rtl">
            <div className="bg-[#0F172A] rounded-xl border border-slate-800 overflow-hidden shadow-2xl">
                <div className="flex items-center justify-between p-5 border-b border-slate-800">
                    ...
                    <h2 className="text-xl font-bold text-white">إدارة الفواتير</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-right">
                        <thead className="bg-slate-800/50 text-slate-400 border-b border-slate-800">
                            <tr>
                                <th className="p-4">#</th><th className="p-4">العميل</th><th className="p-4">التاريخ</th>
                                <th className="p-4">الإجمالي</th><th className="p-4">المدفوع</th><th className="p-4">الحالة</th><th className="p-4 text-center">إجراءات</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800">
                            {loading ? <tr><td colSpan={7} className="p-8 text-center text-slate-500">جاري التحميل...</td></tr>
                            : filtered.length === 0 ? <tr><td colSpan={7} className="p-8 text-center text-slate-500">لا توجد فواتير.</td></tr>
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.amount_paid} onChange={e => setFormData({ ...formData, amount_paid: e.target.value })}
                                        className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500" dir="ltr" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm text-slate-400 mb-1.5">نسبة الضريبة (%)</label>
                                    <input type="text" value={formData.tax_rate} onChange={e => setFormData({ ...formData, tax_rate: e.target.value })}
                                        className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500" dir="ltr" />
                                </div>
                                <div>
