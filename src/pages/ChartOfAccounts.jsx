                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-right">
                        ...
                        <tbody className="divide-y divide-slate-800/70">
                            {loading ? (
                                <tr><td colSpan={5} className="p-10 text-center text-slate-500">جاري التحميل...</td></tr>
                            ) : filtered.length === 0 ? (
                                <tr><td colSpan={5} className="p-10 text-center text-slate-500">لا توجد حسابات.</td></tr>
                            ) : filtered.map((item, idx) => (
                                <tr key={item.id} className="hover:bg-slate-800/30 transition-colors">
                                    <td className="p-4 text-slate-500">{idx + 1}</td>
                                    <td className="p-4 font-semibold text-white">{item.account_name}</td>
                                    <td className="p-4 text-slate-300">{item.account_type || '-'}</td>
                    ...
                    <div className="bg-[#0F172A] border border-slate-700 rounded-xl w-full max-w-lg shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            <div>
                                ...
                                <input required type="text" value={form.account_name} onChange={e => setForm({ ...form, account_name: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm text-slate-400 mb-1.5">النوع</label>
                                    <input type="text" placeholder="أصول، خصوم، إيرادات..." value={form.account_type} onChange={e => setForm({ ...form, account_type: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" />
                                </div>
                                <div>
            ...
            {deleteModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-[#0F172A] border border-slate-700 rounded-xl w-full max-w-sm shadow-2xl p-6 text-center">
                        ...
                        <p className="text-slate-400 mb-6 text-sm">هل أنت متأكد من حذف حساب <span className="text-white font-semibold">{itemToDelete?.account_name}</span>؟</p>
