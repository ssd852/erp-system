    ...
    return (
        <div className="p-6 w-full min-h-screen bg-[#0B1120]" dir="rtl">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-white">إدارة العملاء</h2>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => { openAdd(); setShowModal(true); }} className="bg-[#22C55E] hover:bg-[#16a34a] text-white px-5 py-2.5 rounded-lg flex items-center gap-2 font-bold transition-colors">
                        ...
                        <tbody className="divide-y divide-slate-800/70">
                            {loading ? (
                                ...
                                <tr key={item.id} className="hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="p-4 text-slate-300" dir="ltr">{item.phone || '-'}</td>
                                    <td className="p-4 text-slate-300">{item.email || '-'}</td>
                                    <td className="p-4 text-slate-300">{item.city || '-'}</td>
                                    <td className="p-4 text-emerald-400 font-mono font-bold">${Number(item.balance || 0).toLocaleString()}</td>
                                    <td className="p-4">
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" />
                                </div>
                                <div>
                                    <label className="block text-sm text-slate-400 mb-1.5">البريد الإلكتروني</label>
                                    <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm text-slate-400 mb-1.5">المدينة</label>
                                    <input type="text" value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" />
                                </div>
                                <div>
                                    <label className="block text-sm text-slate-400 mb-1.5">الرصيد</label>
                                    <input type="text" value={form.balance} onChange={e => setForm({ ...form, balance: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" dir="ltr" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm text-slate-400 mb-1.5">العنوان</label>
                                <input type="text" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors" />
                            </div>
